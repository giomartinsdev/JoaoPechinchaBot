...
class OffersPoller {
  ...
  private readonly pollerInterval2: number = parseInt(process.env.POLL_INTERVAL || '300000', 10);
  ...
  async fetchLatestOffers(): Promise<void> {
    try {
      const apiResponse = await axios.post(this.pollerUrl2, JSON.stringify({ query }), { headers });

      if (apiResponse.data && apiResponse.data.data.offers && apiResponse.data.data.offers.items) {
        ...
        if (latestOffer) {
          if (this.lastOfferId !== latestOffer.id) {
            ...
            console.log('The actual offer is the same as the last, message not sent.');
          }
        } else {
